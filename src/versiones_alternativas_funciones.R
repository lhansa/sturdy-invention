...
# Col %>% length
# Col %>% unique %>% length
# unique(Col[Col %in% colores])
# 
# colvariable_bis(colvar,col,NAcol, clim, NULL)
...
scatter3D <- function (x, y, z, ..., colvar = z, phi = 40, theta = 40, col = NULL, 
          ...
          add = FALSE, plot = TRUE) 
{
  plist <- plot3D:::initplist(add)
  dot <- plot3D:::splitdotpersp(list(...), bty, NULL, x, y, z, plist = plist, 
                       breaks = breaks)
  x <- as.vector(x)
  y <- as.vector(y)
  z <- as.vector(z)
  ...
  if (len > 1 & plot3D:::ispresent(colvar)) {
    ...
    if (is.null(col)) 
      if (is.null(breaks)) 
        col <- jet.col(100)
    else col <- jet.col(length(breaks) - 1)
    iscolkey <- plot3D:::is.colkey(colkey, col)
    if (iscolkey) 
      colkey <- check.colkey(colkey)
    if (!is.null(dot$alpha)) 
      col <- plot3D:::setalpha(col, dot$alpha)
  ...
  breaks <- plot3D:::check.breaks(breaks, col)
  fit <- NULL
  if (!is.null(surf)) {
    if (!is.list(surf)) 
  ...
  if (dtype == "h") {
    ...
    Proj <- plot3D:::project(x, y, 0.5 * (z + zmin), plist, FALSE)
    ...
    Proj <- plot3D:::project(0.5 * (x[-len] + x[-1]), 0.5 * (y[-len] + 
  ...
  if (!is.null(CI)) {
    ...
    if (is.null(CIpt$CIpar$lty)) 
      CIpt$CIpar$lty <- 1
    CIpt$CIpar <- plot3D:::setdots(CIpt$CIpar, len)
    ...
    pt <- list(x.mid = x[ii], y.mid = y[ii], z.mid = z[ii], 
               ...
               lwd = rep(lwd, length.out = length(ii)), cex = rep(cex, 
                                                                  length.out = length(ii)), bg = rep(bg, length.out = length(ii)), 
               alpha = alpha)
    class(pt) <- "pt"
  }
  Proj <- plot3D:::project(x, y, z, plist)
  if (!is.null(CI)) 
    CIpt$proj <- Proj
  else if (!is.null(pt)) 
    pt$proj <- Proj
  if (iscolkey) 
    plist <- plot3D:::plistcolkey(plist, colkey, col, clim, clab, 
                         dot$clog, type = "scatter3D", breaks = breaks)
  plist <- plot3D:::plot.struct.3D(plist, pt = pt, CIpt = CIpt, poly = spoly, 
                          segm = sseg, plot = plot)
  plot3D:::setplist(plist)
  invisible(plist$mat)
}